    ...
    <div className="min-h-screen">
      ...
      <main className="page-shell">
        ...
        <section className="mt-8 grid gap-4 md:grid-cols-3">
          {[{ icon: Activity, title: "Operational picture", text: "Live environmental parameters and sensor health at every active station.", color: "#087f7b" }, { icon: AlertTriangle, title: "Early warning layer", text: "HAB, thermal anomaly and pollution signals surfaced by location.", color: "#d75b4d" }, { icon: ShieldCheck, title: "Trusted provenance", text: "Institution, collection time and data quality travel with each observation.", color: "#69933d" }].map((item) => <div key={item.title} className="content-surface flex gap-4 p-5"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl" style={{ color: item.color, backgroundColor: `${item.color}14` }}><item.icon className="h-5 w-5" /></div><div><h3 className="font-semibold text-[#153d46]">{item.title}</h3><p className="mt-1 text-xs leading-5 text-[#6c8489]">{item.text}</p></div></div>)}
        </section>
      </main>
    </div>
  )
